...
create.adaptive.blockwise.metropolis.control <- function(var.names,
                                                         ...
                                                         adaptive.scaling.update.decay=0.5,
                                                         target.acceptance.probability=0.234,
                                                         initial.scaling.parameters=2.38^2/length(var.names))
{
    ...
    # Check initial scaling parameters
...
#Do we want to double check the chain state elements, or just assume they have not been messed with?
    ...
    for (iter.inc in (1:n.iter))
    {
        ...
        if (print.updates)
        {
            ...
            if (update.detail=='med' && total.iter.inc>1)
            {
            ...
            if (update.detail=='high' && control@n.blocks>1)
            {
        ...
        scaling.submatrix = exp(chain.state@log.scaling.parameters[block.vars]/2) %*% t(exp(chain.state@log.scaling.parameters[block.vars]/2))
        block.cov.mat = zero.matrix
        block.cov.mat[block.vars,block.vars] = scaling.submatrix * chain.state@cov.mat[block.vars,block.vars]

        #-- Print Details --#
        if (print.updates && update.detail=='high')
        {
        ...
        proposed.transformed.parameters = mvtnorm::rmvnorm(1, mean=current.transformed.parameters, sigma=block.cov.mat)[1,]
        ...
        {
            n.each.block = floor(iter / control@n.blocks) + as.numeric(1:control@n.blocks <= iter%%control@n.blocks)
            i.scales = sapply(block.vars, function(var){
                sum(var.is.in.block[,var] * n.each.block)
            })

            scale.updates.step = control@adaptive.scaling.base.update / (control@adaptive.scaling.update.prior.iter + i.scales) ^ control@adaptive.scaling.update.decay

            chain.state@log.scaling.parameters[block.vars] = chain.state@log.scaling.parameters[block.vars] + scale.updates.step *
                (acceptance.probability - control@target.acceptance.probability)
        }
