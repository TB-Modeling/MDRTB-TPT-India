...
#'
#'@export
mcmc.merge.serial <- function(mcmc.objects)
{
    # Check classes
    if (all(class(mcmc.objects)!='list'))
        stop("mcmc.objects must be a list of objects of class 'mcmcsim")

    not.mcmcsim = sapply(mcmc.objects, function(obj){all(class(obj)!='mcmcsim')})
    if (any(not.mcmcsim))
        stop("All elements of mcmc.objects must be of class 'mcmcsim' or a descendant")
    ...
    for (mcmc in mcmc.objects)
    {
        ...
        first.sim.is.repeat.by.chain = iter.inc > 0 &
            sapply(1:n.chains, function(chain){
                if (iter.inc==1)
                    all(mcmc@samples[chain,1,] == samples[chain,1,])
                else if (n.var==1)
...
#'
#'@export
mcmc.merge.parallel <- function(mcmc.objects)
{
